...
st.title("🚚 Ersan Dizayn Rota Optimizasyon Paneli")
st.markdown("Sipariş Excel'inizi yükleyin, Başlangıç ve Bitiş noktalarınızı dilediğiniz gibi ayarlayın.")
...
with col1:
    api_key = st.text_input("Google Maps API Anahtarınız:", type="password")
with col2:
    yuklenen_dosya = st.file_uploader("Sipariş Excel'i Yükle (.xlsx)", type=["xlsx"])

# EXCEL YÜKLENDİKTEN SONRA AÇILAN YENİDEN PLANLAMA MENÜSÜ
if yuklenen_dosya:
    st.markdown("---")
    st.markdown("### 🔄 Rota Ayarları (Yeniden Planla)")
    
    # Exceli Okuma
    df_excel = pd.read_excel(yuklenen_dosya, usecols="H,I,J,P")
    ...
    with col_ayar2:
        # Checkbox'ı varsayılan olarak kapalı yaptık ki Bitiş menüsü hemen görünsün
...
# 4. ALT BÖLÜM: SONUÇLARI GÖSTERME
if st.session_state.harita_hazir:
    st.success("✅ Rota, belirlediğiniz başlangıç/bitiş ayarlarına göre optimize edildi!")
    
    harita_sutunu, liste_sutunu = st.columns([6, 4])
    
    with harita_sutunu:
        folium_static(st.session_state.m, width=800, height=600)
        st.download_button(
    ...
    with liste_sutunu:
        st.markdown("### 📱 Şoför Ekranı (Teslimat Sırası)")
